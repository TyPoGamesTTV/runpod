#!/usr/bin/env python3
"""
768x768 RGB Frame Extractor for A40
Extracts color frames for better content detection
"""
import cv2
import numpy as np
...
class RGBFrameExtractor:
    def __init__(self, resolution=768, num_frames=16):
        self.resolution = resolution
        self.num_frames = num_frames
        logging.info(f"RGB Extractor: {resolution}x{resolution}, {num_frames} frames, COLOR")

    def extract_video(self, video_path):
        """Extract RGB frames from single video"""
    ...
    def process_dataset(self, input_dir, output_dir):
        ...
        logging.info(f"Total videos to process: {len(all_videos)}")
        ...
        logging.info(f"RGB Extraction Complete! Processed: {processed}, Failed: {failed}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python extract_frames_rgb.py <input_dir> <output_dir>")
        print("Example: python extract_frames_rgb.py sample_dataset frames_768_rgb")
        sys.exit(1)
    
    input_dir = sys.argv[1]
    output_dir = sys.argv[2]
    
    extractor = RGBFrameExtractor(resolution=768, num_frames=16)
