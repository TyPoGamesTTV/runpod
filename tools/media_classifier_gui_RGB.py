#!/usr/bin/env python3
"""
Media Classifier GUI - RGB VERSION
For 768x768 RGB (color) models
"""
...
from inference.single_video_rgb import VideoClassifier_RGB

class MediaClassifierGUI_RGB:
    def __init__(self, root):
        self.root = root
        self.root.title("Media Classifier - 768x768 RGB Model")
        self.root.geometry("900x600")
        
        # Set window background to indicate RGB mode
        self.root.configure(bg='#f0f8ff')  # Light blue tint for RGB
    ...
    def create_widgets(self):
        ...
        # RGB Mode Label
        rgb_label = ttk.Label(main_frame, text="RGB COLOR MODEL", 
                             font=('Arial', 12, 'bold'), foreground='blue')
        rgb_label.grid(row=0, column=0, columnspan=2, pady=(0, 10))
        ...
        # Model selection
        ttk.Label(left_frame, text="RGB Model File (.pth):", font=('Arial', 10, 'bold')).grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        self.start_button = ttk.Button(button_frame, text="Start RGB Classification", command=self.start_processing, 
        ...
        # Status label
        self.status_label = ttk.Label(left_frame, text="Ready (RGB Mode)", foreground="green")
        ...
        ttk.Label(right_frame, text="RGB Processing Log:", font=('Arial', 10, 'bold')).grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
    ...
    def browse_model(self):
        filename = filedialog.askopenfilename(
            title="Select RGB Model File",
            filetypes=[("PyTorch Model", "*.pth"), ("All Files", "*.*")]
        )
        if filename:
            self.model_path.set(filename)
            self.log("INFO", f"RGB Model selected: {Path(filename).name}")
    ...
    def validate_inputs(self):
        if not self.model_path.get():
            messagebox.showerror("Error", "Please select an RGB model file")
            return False
        
        if not Path(self.model_path.get()).exists():
            messagebox.showerror("Error", "RGB model file does not exist")
            return False
    ...
    def start_processing(self):
        ...
        self.status_label.config(text="Processing RGB...", foreground="blue")
    ...
    def process_videos(self):
        try:
            # Load RGB model
            self.log("INFO", "Loading RGB model...")
            classifier = VideoClassifier_RGB(self.model_path.get(), device=None)
            self.log("SUCCESS", "RGB model loaded successfully")
            ...
            self.log("INFO", f"Found {self.total_videos} videos to process with RGB model")
            ...
            output_folders = {
                '1_Safe': output_base / '1_Safe_RGB',
                '2_Unsafe': output_base / '2_Unsafe_RGB',
                '3_Explicit': output_base / '3_Explicit_RGB'
            }
            
            if self.create_uncertain.get():
                output_folders['Uncertain'] = output_base / '0_Uncertain_RGB'
            
            for folder in output_folders.values():
                folder.mkdir(parents=True, exist_ok=True)
            
            self.log("INFO", f"Created RGB output folders in {output_base}")
            ...
            for i, video_path in enumerate(video_files):
                ...
                try:
                    # Classify video with RGB
                    self.log("INFO", f"Processing (RGB): {video_path.name}")
            ...
            self.log("SUCCESS", f"RGB processing complete! Processed {self.processed_videos}/{self.total_videos} videos")
            self.log("INFO", f"Results saved to: {output_base}")
            
        except Exception as e:
            self.log("ERROR", f"Fatal error: {str(e)}")
            messagebox.showerror("Error", f"RGB processing failed: {str(e)}")
            ...
            self.status_label.config(text="Ready (RGB Mode)", foreground="green")

def main():
    root = tk.Tk()
    app = MediaClassifierGUI_RGB(root)
    
    # Set icon if available
    try:
        if sys.platform.startswith('win'):
            root.iconbitmap(default='icon_rgb.ico')
    except:
        pass
